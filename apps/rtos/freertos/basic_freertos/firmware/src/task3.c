...
/******************************************************************************
  ...
  Remarks:
    Task3 blocks on a serial data semaphore. This semaphore is released from UART 
	read event handler when a character is received. Once the task is unblocked it
	reads the received characters from the UART ring buffer and toggles an LED if 
	the received character is 'L' or 'l'. It then does work for 50 ticks (1 tick is
	configured as 1 msec). After this it again blocks on the serial data semaphore.
 */
